...
with saas_tab:
    ...
    with ltv_col:
        ...
        saas_arpa = st.number_input("Average Revenue Per Account (ARPA) - Monthly ($)", min_value=0.0, value=250.0, step=10.0, help="The average amount of money you get from one customer each month. (Total Monthly Recurring Revenue / Total Customers).")
        saas_gross_margin = st.slider("Gross Margin (%)", 0.0, 100.0, 80.0, 0.1, format="%.1f%%", help="The percentage of revenue left after paying for the 'cost of goods sold' (COGS). For SaaS, this is often hosting, support, and third-party API costs.")
        saas_churn_rate = st.slider("Customer Churn Rate (%) - Monthly", 0.0, 100.0, 5.0, 0.1, format="%.1f%%", help="The percentage of customers who cancel their subscription each month. (Customers who canceled this month / Total customers at start of month).")
    ...
    with cac_col:
        ...
        saas_spend = st.number_input("Total Sales & Marketing Spend (Monthly, $)", min_value=0, value=10000, step=500, help="The total cost for one month, including salaries, ad spend, software tools, etc.")
        saas_new_customers = st.number_input("New Customers Acquired (Monthly)", min_value=0, value=20, step=1, help="The total number of new customers you won in that same period.")
    ...
    # --- Trend Analysis Section for SaaS ---
    st.divider()
...
with services_tab:
    ...
    with ltv_col:
        ...
        serv_annual_value = st.number_input("Average Annual Client Value ($)", min_value=0.0, value=50000.0, step=1000.0, help="The average amount a single client pays you over a 12-month period. (Total Annual Revenue / Number of Clients).")
        serv_gross_margin = st.slider("Gross Margin (%)", 0.0, 100.0, 40.0, 0.1, format="%.1f%%", key="serv_gm", help="The percentage of revenue left after paying for the 'cost of service delivery,' which includes your team's salaries for billable work.")
        serv_lifespan = st.number_input("Average Client Lifespan (Years)", min_value=0.0, value=3.0, step=0.5, help="The average number of years a client stays with your firm. A good starting point is often 2-3 years.")
    ...
    with cac_col:
        ...
        serv_spend = st.number_input("Total Sales & Marketing Spend (Quarterly, $)", min_value=0, value=30000, step=500, key="serv_spend", help="The total cost for a period (e.g., a quarter), including salaries, ad spend, software tools, etc.")
        serv_new_customers = st.number_input("New Clients Acquired (Quarterly)", min_value=0, value=5, step=1, key="serv_new_customers", help="The total number of new clients you won in that same period.")
    ...
    if serv_cac > 0:
        ...
        res_col3.metric("LTV to CAC Ratio", f"{serv_ratio:,.2f} : 1")
    ...
    # --- Trend Analysis Section for Services ---
    st.divider()
    st.subheader("📈 LTV:CAC Trend Analysis")
    st.markdown("Analyze how your acquisition efficiency has changed over time. Your calculated LTV above will be used to calculate the ratio for each period.")
    ...
    if serv_ltv > 0:
        trend_data_serv = {
            ...
            'Total Sales & Marketing Spend ($)': [30000, 32000, 28000, 35000],
            'New Clients Acquired': [5, 6, 4, 7]
        }
        trend_df_serv = pd.DataFrame(trend_data_serv)
        
        st.write("**Enter your historical data for each period below:**")
        edited_df_serv = st.data_editor(trend_df_serv, num_rows="dynamic")

        edited_df_serv['CAC ($)'] = edited_df_serv['Total Sales & Marketing Spend ($)'] / edited_df_serv['New Clients Acquired']
        edited_df_serv['LTV to CAC Ratio'] = serv_ltv / edited_df_serv['CAC ($)']
        
        st.write("**Trend Results:**")
        st.line_chart(edited_df_serv.set_index('Period')['LTV to CAC Ratio'])
        
        with st.expander("Show Trend Data Table"):
            st.dataframe(edited_df_serv)
    else:
        st.warning("Please calculate a valid LTV in the section above to enable the trend analysis.")
...
with one_off_tab:
    ...
    with ltv_col:
        ...
        one_off_margin_value = st.number_input("Gross Margin on Initial Sale ($)", min_value=0.0, value=500000.0, step=1000.0, help="The total profit (in dollars) from the primary sale. (Sale Price - Cost of Goods Sold).")
        one_off_service_revenue = st.number_input("Expected Annual Service Revenue ($)", min_value=0.0, value=25000.0, step=500.0, help="The average revenue you expect from service contracts or spare parts per year.")
        one_off_service_margin = st.slider("Gross Margin on Service Revenue (%)", 0.0, 100.0, 60.0, 0.1, format="%.1f%%", help="The profit margin on your service and parts business, which is often high.")
        one_off_service_lifespan = st.number_input("Expected Lifespan of Service (Years)", min_value=0.0, value=5.0, step=0.5, help="How many years you expect to retain the service contract.")
    ...
    with cac_col:
        ...
        one_off_spend = st.number_input("Total Sales & Marketing Spend (Annually, $)", min_value=0, value=200000, step=1000, key="one_off_spend", help="The total cost for a long sales cycle (e.g., a year), including salaries, travel, trade shows, etc.")
        one_off_new_customers = st.number_input("New Customers Acquired (Annually)", min_value=0, value=2, step=1, key="one_off_new_customers", help="The total number of new customers you won in that same period.")
    ...
    if one_off_cac > 0:
        ...
        res_col3.metric("LTV to CAC Ratio", f"{one_off_ratio:,.2f} : 1")
    ...
    # --- Trend Analysis Section for High-Value Sale ---
    st.divider()
    st.subheader("📈 LTV:CAC Trend Analysis")
    st.markdown("Analyze how your acquisition efficiency has changed over time. Your calculated LTV above will be used to calculate the ratio for each period.")
    ...
    if one_off_ltv > 0:
        trend_data_one_off = {
            'Period': ['2022', '2023', '2024', '2025 (Forecast)'],
            'Total Sales & Marketing Spend ($)': [180000, 200000, 220000, 240000],
            'New Customers Acquired': [2, 2, 3, 3]
        }
        trend_df_one_off = pd.DataFrame(trend_data_one_off)
        
        st.write("**Enter your historical data for each period below:**")
        edited_df_one_off = st.data_editor(trend_df_one_off, num_rows="dynamic")

        edited_df_one_off['CAC ($)'] = edited_df_one_off['Total Sales & Marketing Spend ($)'] / edited_df_one_off['New Customers Acquired']
        edited_df_one_off['LTV to CAC Ratio'] = one_off_ltv / edited_df_one_off['CAC ($)']
        
        st.write("**Trend Results:**")
        st.line_chart(edited_df_one_off.set_index('Period')['LTV to CAC Ratio'])
